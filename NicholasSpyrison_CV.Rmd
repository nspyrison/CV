...
```{r results='asis'}
tribble(
  ~Start, ~End, ~Degree, ~Institution, ~Details,
  2018, "2021 (expected)", "Ph.D. Informaiton Technology", "Monash University, Clayton, VIC AUS", "Thesis: New tools user-guided exploration of linear projections", 
  2008, 2012, "B.Sc. Statistics. Minors: mathematics and physics", "Iowa State University, Ames, IA USA", "Minors: physics and mathematics"
) %>%
  arrange(-Start) %>%
  mutate(End = replace_na(End, "")) %>%
  mutate(Years = paste(Start, "--", End, sep = "")) %>%
  select(Years, Degree, Institution) %>% #, #Details) %>%
  baretable()
```
